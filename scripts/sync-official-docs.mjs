...
2. If unsure whether a built-in exists, check \`reference/INDEX.md\`. Never use a name that isn't listed there. Each built-in is a \`### name()\` heading (variables: \`### name\`), so grep for it directly, e.g. \`grep -n -A40 '^### ta.rsi()' reference/ta.md\`.
