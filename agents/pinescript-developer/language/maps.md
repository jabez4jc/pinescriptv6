...
Pine Script *maps* are [collections](type_system.md#collections) that store data in *key-value pairs*. They enable scripts to collect multiple values or references in a single location and associate those elements with specific *unique values (keys)*.

In contrast to [arrays](arrays.md) and [matrices](matrices.md), maps are *unordered* collections. Scripts do not access a map’s value elements by traversing an internal index. Instead, they quickly access values by using the *keys* from the collection’s key-value pairs as *lookup indices*.

A map’s key elements must be of a [value type](type_system.md#value-types), including any *fundamental type* or [enum type](type_system.md#enum-types). Maps *do not* allow the IDs of any [reference type](type_system.md#reference-types) as keys, but they do allow those IDs as *value elements*. Note that, as with other collections, maps cannot directly store the IDs of other collections. However, their value elements can store references to objects of [user-defined types](type_system.md#user-defined-types) that have collection-type fields. See the [Maps of other collections](#maps-of-other-collections) section to learn more.
...
- Replacing the pairs in a map does not change the internal *insertion order* of its keys. We discuss this further in the [next section](#inspecting-keys-and-values).

Similar to working with other collections, when putting the reference for an instance of a *special type* (`line`, `linefill`, `box`, `polyline`, `label`, `table`, or `chart.point`) or a [user-defined type](type_system.md#user-defined-types) into a map, it's important to note the inserted pair's `value` refers to that same object without copying it. Modifying the object referenced by a key-value pair directly modifies the *original* object.
...
- This script would behave differently if it passed a copy of `myData` into each `map.put()` call. For more information, see the [Copying objects](objects.md#copying-objects) section of the [Objects](objects.md) page.
...
Although maps are *unordered* collections, Pine Script internally maintains the *insertion order* of a map's key-value pairs. As a result, the `map.keys()` and `map.values()` functions always return [arrays](arrays.md) with their elements ordered based on the `id` map's insertion order.
...
It's important to note a map's internal insertion order **does not** change when replacing its key-value pairs. The locations of the new elements in the `map.keys()` and `map.values()` arrays will be the same as the old elements in such cases. The only exception is if the script completely [removes](#removing-key-value-pairs) the key beforehand.
...
To remove a specific key-value pair from an `id` map, use `map.remove()`. This function removes the `key` and its associated value from the map while preserving the insertion order of other key-value pairs. It returns the removed value if the map [contained](#mapcontains) the specified `key`. Otherwise, it returns `na`.

To remove all key-value pairs from an `id` map at once, use `map.clear()`.

The following script creates a new `m` map, [puts](#putting-and-getting-key-value-pairs) key-value pairs into the map, uses a `map.remove()` method call within a loop to remove each valid `key` listed in the `removeKeys` array, then calls `map.clear()` as a method to remove all remaining key-value pairs. The script uses a custom `debugLabel()` method to display the `map.size()`, `map.keys()`, and `map.values()` results for the map after each change:
...
This example contains a user-defined `hexMap()` function that maps decimal `int` keys to `string` representations of their [hexadecimal](https://en.wikipedia.org//wiki/Hexadecimal) forms. The script uses this function to create two maps, `mapA` and `mapB`, then calls `map.put_all()` as a method to put all key-value pairs from `mapB` into `mapA`.
...
However, we recommend using a [`for…in` loop](loops.md#forin-loops) directly on a map, as it iterates over the map's key-value pairs in their insertion order, returning a tuple containing the next pair's key and value on each iteration.
...
While a [shallow copy](#shallow-copies) will suffice when copying maps that have values of a [fundamental type](type_system.md#types) or [enum type](type_system.md#enum-types), it’s crucial to understand that shallow copies of a map with elements of a *reference type* (`line`, `linefill`, `box`, `polyline`, `label`, `table`, `chart.point` or a [UDT](type_system.md#user-defined-types)) point to the same objects as the original. Modifying the objects referenced by a shallow copy’s elements affect the objects referenced by the original map and vice versa.
...
This example creates an `original` map to store `string` key elements and `label` value elements, then [puts](#putting-and-getting-key-value-pairs) a key-value pair into it. The script copies the map to a `shallow` variable via the built-in `map.copy()` method, then to a `deep` variable using a custom `deepCopy()` method.
...
Maps cannot directly store references to other maps, [arrays](arrays.md), or [matrices](matrices.md) as value elements, but they can hold references to objects of a [user-defined type](type_system.md#user-defined-types) whose fields can reference other collections.
...
The script below uses this concept to construct a map to reference other maps that hold OHLCV data requested from multiple tickers. The user-defined `requestData()` function requests price and volume data from a ticker, creates a `<string, float>` map, [puts](#putting-and-getting-key-value-pairs) the data into it, then returns a `Wrapper` instance containing the new map.
