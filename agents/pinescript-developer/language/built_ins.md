...
- Bar states in the `barstate` namespace (see the [Bar states](../concepts/bar_states.md) page): `barstate.isconfirmed`, `barstate.isfirst`, `barstate.ishistory`, `barstate.islast`, `barstate.islastconfirmedhistory`, `barstate.isnew`, and `barstate.isrealtime`.
...
To learn how to do this, one needs to understand Pine Script's [type system](type_system.md). The Reference Manual entry for each built-in function includes an “ARGUMENTS” section which lists the qualified type required for the argument supplied to each of the function's parameters.
