...
We’ve added a new `once` keyword. It defines a conditional structure that executes a block of code when its condition evaluates to `true`. After the local block executes for the first time on a closed bar, it does not execute again on any subsequent bars, regardless of the condition. For more information, see the [`once` structure](language/conditional_structures.md#once-structure) section of the [Conditional structures](language/conditional_structures.md) page.

#### Binary search in UDT arrays

The `array.binary_search()`, `array.binary_search_leftmost()`, and `array.binary_search_rightmost()` functions can now search [arrays](language/arrays.md) that store IDs of [user-defined types (UDTs)](language/type_system.md#user-defined-types). Like the [sorting functions for UDT collections](#sorting-udt-collections), these functions include a `sort_field` parameter, which specifies the [object](language/objects.md) field that a call compares while searching. The parameter accepts a “const int” *field index* (0 by default, referring to the first field in the type declaration) or a “const string” *field name*. Note that the array must be *sorted* by the same field, in ascending order, for the search to return correct results.

### July 2026

#### Strategy improvements

We’ve added a parameter to the `strategy()` declaration statement for defining a new calculation behavior: [`calc_on_every_history_tick`](concepts/strategies.md#calc_on_every_history_tick). An argument for this parameter must include the parameter’s name (e.g., `calc_on_every_history_tick = true`). If the value is `true`, the script executes once for *each available tick* in *every historical bar*. As the script executes on a historical bar, the values of built-in price and volume variables, such as `high`, `low`, `close`, and `volume`, *update* on each tick to approximate the data that was available when the bar was still developing. This behavior allows for more granular calculations and order fills across a chart’s history, while also reducing the risk of lookahead bias in the simulation. This new feature is available on *standard charts* for users with Premium and Ultimate plans. See the [Altering calculation behavior](concepts/strategies.md#altering-calculation-behavior) section of the [Strategies](concepts/strategies.md) page to learn more.

We’ve also made multiple changes to the “Settings/Properties” tab and the [strategy report](concepts/strategies.md#strategy-report) interface (formerly known as the *Strategy Tester*), including the following:

- The strategy report and the “Properties” tab now include a [Script execution](https://www.tradingview.com/support/solutions/43000786178/) menu, where users can customize the strategy’s allowed executions. The [`calc_on_every_tick`](concepts/strategies.md#calc_on_every_tick), [`calc_on_every_history_tick`](concepts/strategies.md#calc_on_every_history_tick), and [`calc_on_order_fills`](concepts/strategies.md#calc_on_order_fills) parameters of the `strategy()` statement define the default settings enabled in this menu.
- The former “Using bar magnifier” checkbox has been changed to a new [Bar detalization](https://www.tradingview.com/support/solutions/43000786180/) menu, which is available in the “Properties” tab and at the top of the strategy report. This menu replaces the *Bar Magnifier* feature; it controls the level of intrabar detail available for filling orders and updating calculations on historical bars. The `use_bar_magnifier` parameter defines the strategy’s default “Bar detalization” setting. See the [Adjusting historical bar detail](concepts/strategies.md#adjusting-historical-bar-detail) section of the [Strategies](concepts/strategies.md) page to learn more.
- The former “Fill orders using standard OHLC” checkbox for specifying whether a strategy uses real prices on [Heikin Ashi](https://www.tradingview.com/support/solutions/43000619436/) charts has been replaced by a *“Heikin Ashi mode”* dropdown input. The input appears in the “Properties” tab only while the strategy runs on a Heikin Ashi chart. The `fill_orders_on_standard_ohlc` parameter defines the default state of this input.
- The former “Verify price for limit orders” input in the “Properties” tab has been replaced by a *“Limit order execution”* dropdown input, with a fixed set of options for defining assumptions about [unfilled limit orders](concepts/strategies.md#slippage-and-unfilled-limits). Programmers can still specify an alternative default assumption by including a `backtest_fill_limits_assumption` argument in the declaration statement.
...
Expressions can use multiline strings as operands and arguments, just like single-line strings. Therefore, programmers can use the multiline string syntax to create unique [line wrapping](language/script_structure.md#line-wrapping) formats in their code. For example:
...
See the [Multiline strings](concepts/strings.md#multiline-strings) section of the [Strings](concepts/strings.md) page to learn more about multiline strings and how they differ from single-line strings.
...
The `array.sort()`, `array.sort_indices()`, and `matrix.sort()` functions can now sort [arrays](language/arrays.md) and [matrices](language/matrices.md) that store IDs of [user-defined types (UDTs)](language/type_system.md#user-defined-types). These functions sort UDT collections by comparing values from one of the “int”, “float”, or “string” *fields* in the [objects](language/objects.md) referenced by their elements.
...
- A “const int” argument specifies a field by its *field index*, where a value of 0 (the default) refers to the *first* field in the [type declaration](language/type_system.md#user-defined-types).
...
Refer to the [Sorting arrays of user-defined types](language/arrays.md#sorting-arrays-of-user-defined-types) section of the [Arrays](language/arrays.md) page and the [Sorting matrices of user-defined types](language/matrices.md#sorting-matrices-of-user-defined-types) section of the [Matrices](language/matrices.md) page to learn more about sorting UDT collections and the `sort_field` parameter.

### January 2026

#### Footprint requests

We’ve added a new `request.footprint()` function and two new *data types*, `footprint` and `volume_row`. These features enable scripts to retrieve and work with [volume footprint](https://www.tradingview.com/support/solutions/43000726164-volume-footprint-charts-a-complete-guide/) data for a chart’s dataset:
...
See the [`request.footprint()`](concepts/other_timeframes_and_data.md#requestfootprint) section of the [Other timeframes and data](concepts/other_timeframes_and_data.md) page to learn more about footprint requests. For more information about the `footprint` and `volume_row` types and the functions in their namespaces, refer to the [footprint and volume_row](language/type_system.md#footprint-and-volume_row) section of the [Type system](language/type_system.md) page.
...
We’ve updated the maximum length for [strings](concepts/strings.md). Previously, a “string” value could not exceed 4,096 characters. Now, strings can contain up to 40,960 encoded characters.
...
With this new update, the closing timestamp of a realtime bar on tick charts or price-based charts is always available immediately after the bar closes. Now, scripts can use `time_close` with the `[]` [history-referencing operator](language/operators.md#-history-referencing-operator) or call `time_close()` with a positive `bars_back` argument to retrieve the closing times of elapsed realtime bars on *any* chart type. For example:
...
To learn more about this new behavior, refer to the [`for` loops](language/loops.md#for-loops) section of the [Loops](language/loops.md) page and the [Dynamic `for` loop boundaries](migration_guides/to_pine_version_6.md#dynamic-for-loop-boundaries) section of the [v6 migration guide](migration_guides/to_pine_version_6.md).

### February 2025

We’ve removed the scope count limit. Previously, any script’s total number of scopes, including the global scope and all local scopes from [user-defined functions](language/user_defined_functions.md) and [methods](language/methods.md#user-defined-methods), [loops](language/loops.md), [conditional structures](language/conditional_structures.md), [user-defined types](language/type_system.md#user-defined-types), and [enums](language/enums.md), was limited to 550. Now, scripts can contain an indefinite number of local scopes from these structures.
...
- The `size` property of [labels](visuals/text_and_shapes.md#labels) and the `text_size` property of [boxes](visuals/lines_and_boxes.md#boxes) and [tables](visuals/tables.md) now support “int” values in addition to the `size.*` constants. These “int” values represent sizes in *typographic points*, offering a more granular and wide range of text size possibilities.
...
- [Strategies](concepts/strategies.md) no longer stop calculating and raise an error when they reach the 9000 trade limit while not using Deep Backtesting mode. Instead, they *trim* the oldest orders to make space for new ones. The trimmed orders are *not* visible in the [strategy report](concepts/strategies.md#strategy-report), but that does not change the strategy’s simulation. To retrieve the trade index of the earliest *non-trimmed* order, use the `strategy.closedtrades.first_index` variable.
- The `array.get()`, `array.set()`, `array.insert()`, and `array.remove()` functions now support *negative* `index` arguments to reference elements starting from the *end* of an [array](language/arrays.md#arrays). For instance, the call `array.get(myArray, -2)` retrieves the second to last element in `myArray`, which is equivalent to `array.get(myArray, array.size(myArray) - 2)`.
...
Enums, also known as *enumerations*, *enumerated types*, or [enum types](language/type_system.md#enum-types), are unique data types with all possible values declared by the programmer. They can help programmers maintain more strict control over the values allowed by variables, conditional expressions, and [collections](language/type_system.md#collections), and they enable convenient dropdown input creation with the new [input.enum()](concepts/inputs.md#enum-input) function. See our User Manual's [Enums](language/enums.md) page to learn more about these new types and how to use them.
...
Our new [Pine Profiler](writing_scripts/profiling_and_optimization.md#pine-profiler) is a powerful utility that analyzes the executions of all significant code in a script and displays helpful performance information next to the code lines *inside* the Pine Editor. The [Profiler](writing_scripts/profiling_and_optimization.md#pine-profiler)'s information provides insight into a script's runtime, the distribution of runtime across significant code regions, and the number of times each code region executes. With these insights, programmers can effectively pinpoint performance *bottlenecks* and ensure they focus on [optimizing](writing_scripts/profiling_and_optimization.md#optimization) their code where it truly matters when they need to improve execution times.

See the new [Profiling and optimization](writing_scripts/profiling_and_optimization.md) page to learn more about the Profiler, how it works, and how to use it to analyze a script's performance and identify optimization opportunities.
...
Polylines are drawings that sequentially connect the coordinates from an `array` of up to 10,000 [chart points](language/type_system.md#chart-points) using straight or *curved* line segments, allowing scripts to draw custom formations that are difficult or impossible to achieve using `line` or `box` objects. To learn more about this new drawing type, see the [Polylines](visuals/lines_and_boxes.md#polylines) section of our User Manual's page on [Lines and boxes](visuals/lines_and_boxes.md).
...
- `request.seed()` - Requests data from a user-maintained GitHub repository and returns it as a series. An in-depth tutorial on how to add new data can be found [here](https://github.com/tradingview-pine-seeds/docs).
...
- `timeframe.from_seconds()` - Converts a specified number of `seconds` into a valid timeframe string based on our [timeframe specification format](concepts/timeframes.md#timeframe-string-specifications).
...
Maps are collections that hold elements in the form of *key-value pairs*. They associate unique keys of a *fundamental type* with values of a *built-in* or [user-defined](language/type_system.md#user-defined-types) type. Unlike [arrays](language/arrays.md), these collections are *unordered* and do not utilize an internal lookup index. Instead, scripts access the values of maps by referencing the *keys* from the key-value pairs put into them. For more information on these new collections, see our [User Manual’s page on Maps](language/maps.md).
...
Pine objects are instantiations of the new user-defined composite types (UDTs) declared using the `type` keyword. Experienced programmers can think of UDTs as method-less classes. They allow users to create custom types that organize different values under one logical entity. A detailed rundown of the new functionality can be found in our [User Manual's page on objects](language/objects.md).
...
New overload for the `fill()` function was added. Now it can create vertical gradients. More info about it in the [blog post](https://www.tradingview.com/blog/en/pine-script-vertical-gradients-33586/).
...
Support for [matrices](language/matrices.md#matrices) has been added to the `request.security()` function.

The historical states of [arrays](language/arrays.md#arrays) and [matrices](language/matrices.md#matrices) can now be referenced with the `[]` operator. In the example below, we reference the historical state of a matrix 10 bars ago:
...
Added `use_bar_magnifier` parameter for the `strategy()` function. When `true`, the [Broker Emulator](concepts/strategies.md#broker-emulator) uses lower timeframe data during history backtesting to achieve more realistic results.
...
- `str.match(source, regex)` - Extracts the substring matching the specified [regular expression](https://en.wikipedia.org/wiki/Regular_expression#Perl_and_PCRE).
...
Pine Script v5 is here! This is a list of the **new** features added to the language, and a few of the **changes** made. See the Pine Script v5 [Migration guide](migration_guides/to_pine_version_5.md) for a complete list of the **changes** in v5.

#### New features

Libraries are a new type of publication. They allow you to create custom functions for reuse in other scripts. See this manual's page on [Libraries](concepts/libraries.md).
...
A v4 to v5 converter is now included in the Pine Script Editor. See the Pine Script v5 [Migration guide](migration_guides/to_pine_version_5.md) for more information on converting your scripts to v5.
...
See the Pine Script v5 [Migration guide](migration_guides/to_pine_version_5.md) for a complete list of the **changes** made in v5.
...
A new `box` drawing has been added to Pine Script, making it possible to draw rectangles on charts using the Pine Script syntax. For more details, see the Pine Script reference entry for `box.new()` and the [Lines and boxes](visuals/lines_and_boxes.md) User Manual page.

The `color.new` function can now accept series and input arguments, in which case, the colors will be calculated at runtime. For more information about this, see our [Colors](visuals/colors.md) User Manual page.
...
Added support for arrays and functions for working with them. You can now use the powerful new array feature to build custom datasets. See our [User Manual page on arrays](language/arrays.md) and our [blog](https://www.tradingview.com/blog/en/arrays-are-now-available-in-pine-script-20052/)
...
- New [label styles](visuals/text_and_shapes.md#positioning-labels) allow you to position the label pointer in any direction.
