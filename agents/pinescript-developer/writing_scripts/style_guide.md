...
The Pine Script® compiler is quite forgiving of the positioning of specific statements or the version [compiler annotation](../language/script_structure.md#compiler-annotations) in the script. While other arrangements are syntactically correct, this is how we recommend organizing scripts:
...
This is the [compiler annotation](../language/script_structure.md#compiler-annotations) defining the version of Pine Script the script will use. If none is present, v1 is used. For v6, use:
...
If your script uses one or more Pine Script [libraries](../concepts/libraries.md), your `import` statements belong here.
...
- Their declaration uses the optional `const` keyword (see our User Manual's section on [type qualifiers](../language/type_system.md#qualifiers) for more information).
...
It will also help readers if you document the function's objective, parameters and result. The same syntax used in [libraries](../concepts/libraries.md) can be used to document your functions. This can make it easier to port your functions to a library should you ever decide to do so:
...
[Line wrapping](../language/script_structure.md#line-wrapping) can make long lines of code easier to read by defining a *single line* of code across *multiple* lines in the script. Generally, scripts can wrap lines using any indentation length that is *not* a multiple of four, because the four-space or tab indentation defines [local blocks](../faq/programming.md#what-does-scope-mean) in Pine.
...
Line wrapping is also useful when working with long [single-line strings](../concepts/strings.md#single-line-strings). For example, instead of defining a lengthy string on a single line of code, programmers can split that string into smaller parts and [concatenate](../concepts/strings.md#concatenation) them using the `+` operator to wrap the expression across multiple lines for readability:
...
Similar line-wrapping behaviors apply to [multiline strings](../concepts/strings.md#multiline-strings), which can span multiple visible lines in the code. All parts of the code between a multiline string’s `"""` or `'''` delimiters, including leading spaces and line breaks, represent *literal text*. Pine treats expressions that use multiline strings as part of a single code line, similar to other types of line-wrapped code. See the [Line wrapping](../language/script_structure.md#line-wrapping) section of the [Script structure](../language/script_structure.md) page for an example.
