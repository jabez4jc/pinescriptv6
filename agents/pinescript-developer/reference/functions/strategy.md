...
The `strategy()` declaration and all `strategy.*` functions. The first section covers July 2026 changes to `strategy()`; per-function entries follow, alphabetically. Built-in `strategy.*` *variables* are in `reference/variables.md` and constants (e.g. `strategy.long`) in `reference/constants.md`.

## strategy() calculation and backtest parameters

Each parameter below sets the **default** for a control in the "Settings/Properties" tab and the strategy report (formerly the Strategy Tester). Users can override it there.

| Parameter | Default | UI control (since July 2026) | Effect |
|---|---|---|---|
| `calc_on_every_tick` | `false` | Script execution → "On realtime bar tick" | Execute on every realtime tick instead of only at bar close. |
| `calc_on_order_fills` | `false` | Script execution → "On order fill" | Execute again on each tick where an order fills (historical and realtime). |
| `calc_on_every_history_tick` | `false` | Script execution → "On history bar tick" | **New.** Execute on every available tick of each historical bar. Requires a named argument. |
| `use_bar_magnifier` | `false` | Bar detalization → "High" | Use lower-timeframe data for intrabar fills and history ticks. Replaces the "Bar Magnifier" checkbox. |
| `process_orders_on_close` | `false` | Order execution delay → "None" | Fill orders on the same bar's closing tick. |
| `fill_orders_on_standard_ohlc` | `false` | Heikin Ashi mode (shown only on HA charts) | Fill using real OHLC prices on Heikin Ashi charts. |
| `backtest_fill_limits_assumption` | `0` | Limit order execution | Ticks that price must move beyond a limit level before it fills. |
| `margin_long` / `margin_short` | `100` | Long leverage / Short leverage | Still margin **percentages** in code; the UI shows them as leverage (e.g., 25 → 4:1). |

### calc_on_every_history_tick

`calc_on_every_history_tick = true` (named argument only) makes the strategy treat historical bars like realtime bars. `high`, `low`, `close`, `volume`, and derived values such as `ohlc4` update on each tick to approximate what was known while the bar was open, which reduces lookahead bias and allows intrabar order fills in history.

- Premium/Ultimate plans and standard chart types only. If enabled by default where unsupported, the strategy raises a runtime error.
- `barstate.isconfirmed` stays `true` throughout historical bars.
- With default bar detail, per-tick values follow the emulator's assumed path: O→H→L→C if open is nearer the high, otherwise O→L→H→C, with `volume` at 25/50/75/100% of the bar total. With "High" bar detalization (`use_bar_magnifier = true`), values come from lower-timeframe bars.
- Can cause repainting and affects `varip`, `log.*()`, alerts, and order commands, like the other execution settings.

```pine
//@version=6
strategy("History tick demo", overlay = true, calc_on_every_history_tick = true, use_bar_magnifier = true)

varip float firstPrice = na
varip int   openTime   = 0
if time != openTime
    openTime   := time
    firstPrice := close   // Price at the bar's FIRST tick when history ticks are enabled.

if close < firstPrice and strategy.position_size == 0
    strategy.entry("Long", strategy.long)
```

See [Altering calculation behavior](https://www.tradingview.com/pine-script-docs/concepts/strategies/#altering-calculation-behavior) and [Margin and leverage](https://www.tradingview.com/pine-script-docs/concepts/strategies/#margin-and-leverage).
...
> **July 2026:** see [strategy() calculation and backtest parameters](#strategy-calculation-and-backtest-parameters) above for `calc_on_every_history_tick` and the renamed Properties-tab controls.
