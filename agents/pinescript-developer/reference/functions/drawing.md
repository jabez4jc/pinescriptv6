# Drawing Functions (Plots, Lines, Boxes, Labels, Polylines, Tables)

Plotting functions (`plot*()`, `fill()`, `hline()`, `bgcolor()`, `barcolor()`) and the `line.*`, `box.*`, `label.*`, `linefill.*`, `polyline.*`, `table.*`, and `chart.point.*` namespaces, alphabetically. For usage patterns see `visuals/*.md`.
