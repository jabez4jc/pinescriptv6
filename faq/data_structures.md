...
A [tuple](../language/type_system.md#tuples) in Pine Script is a list of values that is returned by a [function](../language/user_defined_functions.md), [method](../language/methods.md), or local block. Unlike in other languages, tuples in Pine serve no other function. Tuples do not have names and cannot be assigned to variables. Apart from the fact that the values are requested and returned together, the values have no relation to each other, in contrast to the other data structures described here.
...
See the [Tuples](../language/type_system.md#tuples) section in the User Manual for more information.

### Arrays

[Arrays](../language/arrays.md) store multiple values of the same [type](../language/type_system.md) in a single variable. Each *element* in an array can be efficiently accessed by its *index*—an integer corresponding to its position within the array.
...
See the [Arrays](../language/arrays.md) section in the User Manual for more information.

### Matrices

A [matrix](../language/matrices.md) is a two-dimensional array, made of rows and columns, like a spreadsheet. Matrices, like arrays, store values of the same built-in or user-defined [type](../language/type_system.md#types).
...
This script demonstrates a simple example of matrix addition. It creates a 3x3 matrix, calculates its [transpose](../language/matrices.md#transposing), then calculates the `matrix.sum()` of the two matrices. This example displays [strings](../concepts/strings.md) representing the original matrix, its transpose, and the resulting sum matrix in a [table](https://www.tradingview.com/pine-script-docs/concepts/tables/) on the chart:
...
See the [Matrices](../language/matrices.md) section in the User Manual for more information.

### Objects

Pine Script [objects](../language/objects.md) are containers that group together multiple fields into one logical unit.

Objects are *instances* of [user-defined types](../language/type_system.md#user-defined-types) (UDTs). UDTs are similar to *structs* in traditional programming languages. They define the rules for what an object can contain. Scripts first create a UDT by using the `type` keyword and then create one or more objects of that type by using the UDT’s built-in `new()` method.
...
See the User Manual page on [Objects](../language/objects.md) to learn more about working with UDTs.

### Maps

[Maps](../language/maps.md#maps) in Pine Script are similar to *dictionaries* in other programming languages, such as dictionaries in Python, objects in JavaScript, or HashMaps in Java. Maps store elements as key-value pairs, where each key is unique. Scripts can access a particular value by looking up its associated key.
...
Here, we use `request.earnings()` with the barmerge parameter set to `barmerge.gaps_on` to return the earnings value on bars where earnings data is available, and return `na` otherwise. We add non-na values to the map, associating the dates that earnings occurred with the earnings numbers. Finally, on the last historical bar, the script [loops through the map](../language/maps.md#looping-through-a-map), logging each key-value pair to display the map’s contents.

To learn more about working with maps, refer to the [Maps](../language/maps.md) section in the User Manual.

## What’s the difference between a series and an array?

In Pine Script, [“series”](../language/type_system.md#series) variables are calculated on each bar. Historical values cannot change. Series values can change during the open realtime bar, but when the bar closes, the value for that bar becomes fixed and immutable. These fixed values are automatically indexed for each bar. Scripts can access values from previous bars by using the [[] history-referencing operator](../language/operators.md#-history-referencing-operator) to go back one or more bars.

Where “series” variables are strictly time-indexed, and the historical values are created automatically, [arrays](../language/arrays.md) are created, filled, and manipulated arbitrarily by a script’s logic. Programmers can change the size of arrays dynamically by using functions that [insert or remove elements](../language/arrays.md#inserting-and-removing-array-elements). Any element in an array can also be altered using the `array.set()` function.

The concept of [time series](../language/execution_model.md#time-series) is a fundamental aspect of Pine Script. Its series-based [execution model](../language/execution_model.md) processes scripts bar-by-bar. This built-in behavior mimics looping, allowing a series to track values, accumulate totals, or perform calculations across a sequence of data on each bar.
...
> **Note:** An array itself is part of a series. Scripts can reference the previous committed states of any array by using the history-referencing operator. See the [History referencing](../language/arrays.md#history-referencing) section of the [Arrays](../language/arrays.md) page for more information.

## How do I create and use arrays in Pine Script?

Pine Script [arrays](../language/arrays.md) are one-dimensional collections that can hold multiple values of a single type.

**Declaring arrays**

Declare an array by using one of the following functions: `array.new<type>()`, `array.from()`, or `array.copy()`. Arrays can be declared with the [var](../language/variable_declarations.md#var) keyword to have their values persist from bar to bar, or without it, so that the values initialize again on each bar. For more on the differences between declaring arrays with or without [var](../language/variable_declarations.md#var), see [this section](#whats-the-difference-between-an-array-declared-with-or-without-var) of this FAQ.
...
- `array.insert()` inserts a new element at the specified index and increases the index of existing elements at or after the insertion index by one. It accepts both positive and [negative indices](../language/arrays.md#negative-indexing), which reference an element’s position starting from the beginning of the array or from the end, respectively.
- `array.push()` adds a new element at the end of an array.

**Removing elements**

- `array.remove()` removes the element at the specified index and returns that element’s value. It accepts both positive and [negative indices](../language/arrays.md#negative-indexing), which reference an element’s position starting from the beginning of the array or from the end, respectively.
...
The flexibility afforded by these functions supports various data management strategies, such as [queues](../language/arrays.md#using-an-array-as-a-queue) or [stacks](../language/arrays.md#using-an-array-as-a-stack), which are useful for custom datasets or sliding window calculations. Read more about implementing a stack or queue in [this FAQ entry](#what-are-queues-and-stacks).

**Calculations on arrays**

Because arrays are not [time series](../language/execution_model.md#time-series) data structures, performing operations across an array’s elements requires special functions designed for arrays. Programmers can write custom functions to perform calculations on arrays. Additionally, built-in functions enable computations like finding the maximum, minimum, or average values within an array. See the [Calculation on arrays](../language/arrays.md#calculations-on-arrays) section of the User Manual for more information.

**Script example**

This script example demonstrates a practical application of arrays by tracking the opening prices of the last five sessions. The script declares a float array to hold the prices using the [var](../language/variable_declarations.md#var) keyword, allowing it to retain its values from bar to bar.

At the start of each session, we update the array by adding the new opening price and removing the oldest one. This process, resembling a [queue](../language/arrays.md#using-an-array-as-a-queue), keeps the array’s size constant while maintaining a moving window of the session opens for the last five days. Built-in array functions return the highest, lowest, and average opening price over the last five sessions. We plot these values to the chart.
...
For more information about arrays, see the [Arrays](../language/arrays.md#arrays) page in the User Manual.

## What’s the difference between an array declared with or without ​`var`​?

Using the `var` keyword, a script can declare an [array](../language/arrays.md) variable in a script that is initialized only once, during the first iteration on the first chart bar.
...
For further details, consult the sections concerning variable [declaration modes](../language/variable_declarations.md#declaration-modes) and their use in [array declarations](../language/arrays.md#using-var-and-varip-keywords) in the User Manual.

## What are queues and stacks?

Scripts can use [arrays](../language/arrays.md) to create [queues](../language/arrays.md#using-an-array-as-a-queue) and [stacks](../language/arrays.md#using-an-array-as-a-stack).
...
For more information on manipulating arrays, see the [Arrays](../language/arrays.md) section in the User Manual.
...
If the script’s logic requires the position of the element in the array, use the two-argument form: `for [index, element] in array`. This form returns both the current element and its index in a [tuple](../language/type_system.md#tuples).

**Example: retrieving array elements**

This first example script uses an array as a [queue](../language/arrays.md#using-an-array-as-a-queue) to store [lines](../visuals/lines_and_boxes.md#lines) representing the latest four pivot highs and lows. The `for…in` loop performs two tasks:
...
The following example script checks if the value `3` is present in the `values` array, and displays either “found” or “not found” in a [label](../visuals/text_and_shapes.md#labels).
...
> **Notice:** The `array.binary_search()` function requires arrays of “int” or “float” values, and the values must be [sorted](../language/arrays.md#sorting) in *ascending order* for correct results.
...
For information about commonly encountered array-related errors, refer to the array [Error Handling](../language/arrays.md#error-handling) section in the User Manual.
...
- Arrays must be of type [“float”](../language/type_system.md#float) or [“int”](../language/type_system.md#int).
- The number of plots used for debugging counts towards the [plot limit for a script](../visuals/plots.md#plot-count-limit).
...
Using [labels](../visuals/text_and_shapes.md#labels) to display array values on certain bars is particularly useful for non-continuous data points or to view all elements of an array simultaneously. Scripts can create labels within any local scope, including [functions](../language/user_defined_functions.md) and [methods](../language/methods.md#user-defined-methods). Scripts can also position drawings at any available chart location, irrespective of the current `bar_index`. Unlike plots, labels can display the contents of a variety of array types, including boolean and string arrays.

Limitations of using labels include:

- Pine labels display only in the chart pane.
- Scripts can display only up to a [maximum number of labels](../writing_scripts/limitations.md#line-box-polyline-and-label-limits).
...
Using [tables](../visuals/tables.md) for debugging offers a more organized and scalable alternative to labels. Tables can display multiple [“series”](../language/type_system.md#series) strings in a clear format that remains unaffected by the chart’s scale or the index of the bars.
...
[Pine Logs](../writing_scripts/debugging.md#pine-logs) are messages that display in the Pine Logs pane, along with a timestamp when the logging function was called. Scripts can create log messages at specific points during the execution of a script. Programmers can use the `log.*()` functions to create Pine Logs from almost anywhere in a script – including inside the *local scopes* of [user-defined functions](../language/user_defined_functions.md), [conditional structures](../language/conditional_structures.md), and [loops](../language/loops.md).
...
Pine Script does not directly support multidimensional arrays; however, it provides [matrices](../language/matrices.md) and [user-defined types](../language/type_system.md#user-defined-types) (UDTs). Programmers can use these data structures to create and manipulate complex datasets.

**Matrices**

Pine Script [matrices](../language/matrices.md) are like two-dimensional arrays. They organize data in a rectangular grid, facilitating operations like transformations, linear algebra, and other complex calculations. They are particularly useful for quantitative modeling, such as portfolio optimization, correlation matrix analysis, and more. Just as in [arrays](../language/arrays.md), all elements in a matrix must be of the same [type](../language/type_system.md#types), which can be a built-in or a [user-defined](../language/type_system.md#user-defined-types) type. Pine Script provides a range of functions for [manipulating](../language/matrices.md#manipulating-a-matrix) and performing [calculations](../language/matrices.md#matrix-calculations) on matrices, including addition, subtraction, multiplication, and more.

**Using UDTs for multidimensional structures**

Programmers can achieve similar functionality to multidimensional arrays through defining [user-defined types](../language/type_system.md#user-defined-types) (UDTs). For example, a script can define a UDT that includes an array as one of its fields. UDTs themselves can be contained in arrays. In this way, scripts can effectively have arrays of arrays.

For more information, see the sections on [Matrices](../language/matrices.md), [Maps](../language/maps.md), and [Objects](../language/objects.md) in the User Manual.

## How can I debug objects?

To debug [objects](../language/objects.md), create custom functions that break down an object into its constituent fields and convert these fields into strings. See the [Debugging](../writing_scripts/debugging.md) section of the User Manual for information about methods to display debug information. In particular, [Pine Logs](../writing_scripts/debugging.md#pine-logs) can display extensive and detailed debug information. See the FAQ section about debugging arrays [using Pine Logs](#using-pine-logs) for an explanation of using logs for debugging.

In our example script, we create a [user-defined type](../language/type_system.md#user-defined-types) (UDT) named `openLine`, which includes fields such as `price`, `openTime`, and a line object called `level`. On the first bar of each session, the script initializes a new `openLine` instance. This object tracks the session’s opening price and time, and it draws a line at the open price, extending from the session’s start to its close. An array stores each `openLine` object. A custom function `debugOpenLine()` breaks an `openLine` object into its individual fields, converts the fields to strings, and then logs a message that displays these strings in the console.
