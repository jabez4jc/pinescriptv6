...
- [Labels](../visuals/text_and_shapes.md#labels) and [boxes](../visuals/lines_and_boxes.md#boxes) for dynamic text, which can vary bar to bar.
- [Tables](../visuals/tables.md) for more complex text (static or dynamic) that stays in the same region of the chart.
...
To print different text depending on a logical condition, use two function calls and control them using the `series` parameter. Note that even if the `series` for one or both of the function calls is never true during a script’s execution, and so no shape, character or text is ever plotted, *both* functions still count towards the [plot limit](../writing_scripts/limitations.md#plot-limits).
...
[Labels](../visuals/text_and_shapes.md#labels) are particularly useful for displaying text that can change from one bar to another. The `text` parameter of the `label.new()` function takes a “series string”, so it can change whenever necessary.

Labels do not count towards the [plot limit](../writing_scripts/limitations.md#plot-limits), but there is a separate limit of how many labels can display on the chart. By default, up to approximately 50 of the *most recent* labels appear on the chart. Programmers can adjust this limit up to 500 by setting the `max_labels_count` parameter in the `indicator()` or `strategy()` functions.
...
[Boxes](../visuals/lines_and_boxes.md#boxes) can also display text on the chart, by providing the text to the `text` parameter of the `box.new()` function. Boxes work with text in a similar way to labels, but with some additional features.
...
[Tables](../visuals/tables.md) are useful to display information in a fixed position on the chart. Whereas plots and labels can easily show historical information because they are, or can be, linked to specific bars, table contents do not change as users move the cursor over past chart bars. This makes tables best suited for showing *current* information.
...
Refer to the [Placing a single value in a fixed position](../visuals/tables.md#placing-a-single-value-in-a-fixed-position) section of the [Tables](../visuals/tables.md) page. The example in that section uses a single-cell table to display a string representation of a value in the top-right corner of the chart.
