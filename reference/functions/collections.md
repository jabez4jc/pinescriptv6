# Collection Functions (Arrays, Maps, Matrices)

All `array.*`, `matrix.*`, and `map.*` functions. The first section covers 2026 additions; per-function entries follow, alphabetically.

## Sorting and searching UDT collections (`sort_field`)

Since April 2026, `array.sort()`, `array.sort_indices()`, and `matrix.sort()` accept arrays/matrices of **user-defined type (UDT)** IDs. Since August 2026, `array.binary_search()`, `array.binary_search_leftmost()`, and `array.binary_search_rightmost()` do too. All of them take a `sort_field` parameter that names the object field to compare.

- `sort_field` accepts a **"const int"** field index (`0` = first field in the `type` declaration, the default) or a **"const string"** field name that literally matches a field identifier.
- Only "const" values are allowed; "input", "simple", and "series" values cause a compilation error. To choose the field from an input, use a `switch` with a separate call per field.
- The referenced field must be of type "int", "float", or "string". Pointing at any other field type (including the default field 0) is a compilation error. For example, a UDT whose first field is a `color` can't be sorted without an explicit `sort_field`.
- Binary search requires the array to be sorted **in ascending order by the same field** first.

### array.sort()
`array.sort(id, order, sort_field)`
Sorts the array in place. `order` is `order.ascending` (default) or `order.descending`. `na` values and empty strings go to the end for ascending order and to the beginning for descending order.

### array.sort_indices()
`array.sort_indices(id, order, sort_field) → array<int>`
Returns the indices that would sort the array, without modifying it.

### matrix.sort()
`matrix.sort(id, column, order, sort_field)`
Rearranges the matrix rows by the values in `column`. For UDT matrices, the comparison uses `sort_field` of the object in that column.
...
Returns the index of `val`, or `-1` if it isn't found (`_leftmost` / `_rightmost` return the insertion-point index instead). For UDT arrays, `sort_field` selects the compared field.

> The official docs do not yet show a UDT binary-search example. Check the type expected for `val` on UDT arrays against the [reference manual](https://www.tradingview.com/pine-script-reference/v6/#fun_array.binary_search) before relying on it.

### Code Example
```pine
//@version=6
indicator("Sorting UDT collections")
...
if barstate.islastconfirmedhistory
    array<Data> arr = array.from(Data.new(3.0, 30, "c"), Data.new(1.0, 20, "a"), Data.new(2.0, 10, "b"))
    arr.sort()                                          // By field 0 (`price`).
    arr.sort(sort_field = "timestamp")                  // By field name.
    arr.sort(order.descending, sort_field = 2)          // By field index (`note`).
    array<int> idx = arr.sort_indices(sort_field = "price") // Indices only; `arr` unchanged.
    log.info(str.tostring(idx))
```

See [Sorting arrays of user-defined types](https://www.tradingview.com/pine-script-docs/language/arrays/#sorting-arrays-of-user-defined-types) and [Sorting matrices of user-defined types](https://www.tradingview.com/pine-script-docs/language/matrices/#sorting-matrices-of-user-defined-types).
...
> **2026:** also accepts arrays/matrices of UDT IDs via the `sort_field` parameter. See [Sorting and searching UDT collections](#sorting-and-searching-udt-collections-sort_field) above.
