...
A [compiler annotation](#compiler-annotations) in the following form tells the compiler which of the versions of Pine Script® the script is written in:
...
Notable changes to the current version of Pine Script are documented in the [Release notes](../release_notes.md).
...
- [Strategies](../concepts/strategies.md) must call at least one [order placement command](../concepts/strategies.md#order-placement-and-cancellation) or other output function.
- [Libraries](../concepts/libraries.md) must `export` at least one user-defined [function](user_defined_functions.md), [method](methods.md#user-defined-methods), [type](type_system.md#user-defined-types), or [enum](enums.md).

## Code

Lines in a script that are not [comments](#comments) or [compiler annotations](#compiler-annotations) are *statements*, which implement the script's algorithm. A statement can be one of these:

- [variable declaration](variable_declarations.md)
- [variable reassignment](variable_declarations.md#variable-reassignment)
- [function definition](user_defined_functions.md#structure-and-syntax)
- [built-in function call](built_ins.md#built-in-functions), [user-defined function call](user_defined_functions.md) or [a library function call](../concepts/libraries.md#using-a-library)
- `if`, `for`, `while`, `switch`, `type`, or `enum` *structure*.

Statements can be arranged in multiple ways:

- Some statements can be expressed in one line, like most variable declarations, lines containing only a function call or single-line function declarations. Lines can also be [wrapped](#line-wrapping) (continued on multiple lines). Multiple one-line statements can be concatenated on a single line by using the comma as a separator.
...
We can distribute any part of this single line of code across two or more lines. Within a wrapped statement or expression, the subsequent lines can use different indentation lengths, and can also include [comments](#comments) without disrupting the code:
...
Compiler annotations are [comments](#comments) that issue special instructions for a script:
...
- `//@function`, `//@param` and `//@returns` add custom descriptions for a [user-defined function](user_defined_functions.md) or [method](methods.md), its parameters, and its result when placed above the function declaration.
- `//@type` adds a custom description for a [user-defined type (UDT)](type_system.md#user-defined-types) when placed above the type declaration.
- `//@enum` adds a custom description for an [enum types](type_system.md#enum-types) when placed above the enum declaration.
- `//@field` adds a custom description for the field of a [user-defined type (UDT)](type_system.md#user-defined-types) or an [enum types](type_system.md#enum-types) when placed above the type or enum declaration.
