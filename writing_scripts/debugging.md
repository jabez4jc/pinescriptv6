...
> **Tip:** Effective debugging in the Pine Script environment requires an understanding of the [Execution model](../language/execution_model.md), [Time series](../language/execution_model.md#time-series) structure, and [Type system](../language/type_system.md). We recommend reviewing these topics, along with [string formatting](../concepts/strings.md#string-conversion-and-formatting), which the following techniques often use.
...
Pine [drawings](#pine-drawings) display visuals in the main chart pane or the script’s separate pane. Although they do not output results in other locations, such as the Data Window or Pine Logs pane, drawings provide convenient ways to visualize a script’s data and logic within global or local scopes. [Labels](../visuals/text_and_shapes.md#labels) are the most flexible drawings for debugging, because they can display colored shapes with formatted text and tooltips at any available chart location, for example:
...
> **Notice:** Only *personal* scripts can generate Pine Logs. A published script **cannot** create logs, even if its source code contains `log.*()` function calls. Published [libraries](../concepts/libraries.md) can export functions containing `log.*()` calls for use in personal scripts, but they cannot generate logs directly.
...
- The second overload creates a [formatted string](../concepts/strings.md#formatting-strings) based on its `formatString` and additional arguments, similar to `str.format()`, then displays the resulting text inside the pane.
...
This simple script demonstrates the difference between all three `log.*()` functions. It calls `log.info()`, `log.warning()`, and `log.error()` on the first chart bar to print the values of three [literal strings](../concepts/strings.md#literal-strings) in the Pine Logs pane:
...
- The Pine Logs pane can *filter* messages by their logging level using the menu accessible from the rightmost icon above the logs. See the [Filtering logs](#filtering-logs) section to learn more.
...
During historical executions, scripts log a new message *once* for each `log.*()` call on any bar. During realtime executions, scripts can call the `log.*()` functions to log messages for *any available tick*, regardless of whether the bar is confirmed. The logs created on realtime ticks are *not* subject to [rollback](../language/execution_model.md#executions-on-realtime-bars). All logs remain available in the Pine Logs pane until the script restarts.
...
- The Pine Logs pane can display the most recent 10,000 logs for historical bars. If a programmer needs to view earlier logs, they can add logic in the code to filter specific `log.*()` calls. See the [Custom code filters](#custom-code-filters) section for an example.

The following sections use the example script above to demonstrate the Pine Logs pane’s log [inspection](#inspecting-logs) and [filtering](#filtering-logs) features.

## Inspecting logs

When a script generates a log by calling any `log.*()` function call, the Pine Logs pane automatically prefixes the logged message with an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations) *timestamp* representing the log’s assigned time, expressed in the [chart’s time zone](../concepts/time.md#time-zones). The timestamp prefixed to a log on a historical bar represents the bar’s *opening time*, whereas the timestamp for a realtime log represents the *system time* of the log event:
...
To help users navigate high volumes of logs efficiently, the pane includes *filters* that isolate logs based on [logging level](#logging-level), [start date and time](#start-date), or [search queries](#character-and-pattern-search). Users can apply these log filters individually or in combination to show only the messages that meet specific criteria. The filters are accessible from the icons below the “x” in the top-right portion of the pane:

For custom filtering options, programmers can use conditional logic to activate specific `log.*()` calls selectively across a script’s executions. See the [Custom code filters](#custom-code-filters) section below to learn more.
...
The rightmost icon in the search bar opens a dropdown menu containing three options to adjust the search filter’s behavior: [Match case](#match-case), [Whole word](#whole-word), and [Regex](#regex):
...
For more information about regular expressions, consult the [Regex syntax reference](../concepts/strings.md#regex-syntax-reference) in this manual’s [Strings](../concepts/strings.md) page. Most of the described syntax works the same within the Pine Logs search filter, with a few notable differences:
...
If the filtering options in the Pine Logs pane are not sufficient, programmers can control specific `log.*()` calls using [inputs](../concepts/inputs.md) and conditional logic.
...
The `filterLogsInput`, `logStartInput`, and `logEndInput` variables define a custom *time filter* for generating logs. When `filterLogsInput` is `true`, the script uses the [time inputs](../concepts/inputs.md#time-input) assigned to `logStartInput` and `logEndInput` to filter the `log.info()` calls, allowing a new log only when the bar’s `time` is within the specified range:
...
Pine’s [drawing types](../language/type_system.md#drawing-types) create chart drawings with specified properties. Scripts can place drawings at any valid chart location during code executions on any bar. Programmers can use these types in a script’s global or local scopes to visualize numeric data, conditions, colors, and strings on the chart. The flexibility of Pine drawings makes them helpful for debugging scripts when other methods do not suffice, namely when a programmer wants to inspect information graphically outside the [Pine Logs](#pine-logs) pane.

However, before debugging a script using drawings, it is crucial to note the following limitations:

- The `expression` argument of a `request.*()` call *cannot* depend on code that creates or modifies drawings. Likewise, an indicator that specifies another context in its declaration statement cannot create drawings from *anywhere* in the code. To debug code that executes on requested data, use [Pine Logs](#pine-logs) instead.
...
The sections below explain some simple debugging methods using [labels](../visuals/text_and_shapes.md#labels) and [tables](../visuals/tables.md). These drawings, especially *labels*, are the most effective for on-chart debugging because they can use dynamic [strings](../concepts/strings.md) to express information from other data types as custom text.

### Labels

[Labels](../visuals/text_and_shapes.md#labels) display colored shapes and text at specified chart coordinates. In contrast to the outputs of the `plotshape()` and `plotchar()` functions, labels can display text from “series string” values that change across script executions. Programmers often use labels to visualize the logic of [conditional structures](../language/conditional_structures.md) and show text representing information from a script’s global or local scopes.
...
When inspecting values of varying magnitudes or different types across bars, a simple approach is to create [formatted strings](../concepts/strings.md#formatting-strings) containing the necessary debug information and display them in [labels](../visuals/text_and_shapes.md#labels) on each bar requiring analysis.

In this example, we’ve modified the “Average bar ratio” script from the [Pine Logs](#pine-logs) section above. Instead of creating formatted text and displaying information using `log.*()` function calls, this script formats the values separately, then calls `label.new()` to show the results on the chart within labels anchored to each bar’s `high`:
...
- Unlike the example in the [Pine Logs](#pine-logs) section, this script’s outputs **are** subject to *rollback*, meaning the information shown on a bar reflects only the bar’s *latest* data. The script does **not** show information for *all* realtime bar updates.
...
- The script *restarts* each time the [UNIX timestamps](../concepts/time.md#unix-timestamps) of the `chart.left_visible_bar_time` or `chart.right_visible_bar_time` variables change after the user scrolls or zooms on the chart.

#### Drawing at the end of the chart

When debugging information does not change frequently across executions, or only the information from a specific execution requires inspection, programmers often display it using [labels](../visuals/text_and_shapes.md#labels) anchored to the *end* of the chart.
...
[Tables](../visuals/tables.md) display text within cells arranged in columns and rows at fixed locations in the chart pane’s visual space. In contrast to other [drawing types](../language/type_system.md#drawing-types), which create visuals on the chart at specified *coordinates*, tables appear at one of nine unique, *bar-agnostic* locations defined by the `position.*` constants.
...
- This script calls `printTable()` only on the last historical bar and all realtime bars because updating tables on each historical bar is an unnecessary use of runtime resources. See the [Reducing drawing updates](profiling_and_optimization.md#reducing-drawing-updates) section of the [Profiling and optimization](profiling_and_optimization.md) page for more information.
...
- Unlike [Pine Logs](#pine-logs) or [drawings](#pine-drawings), these outputs *cannot* display results for values that are accessible from local scopes only. Scripts must [extract](#extracting-data-from-local-scopes) values from local scopes into the *global scope* to debug them with plots or chart colors.
- The only `plot*()` functions that can display *text* on the chart – `plotchar()` and `plotshape()` – require “const string” values. Therefore, they cannot display dynamic strings or calculated [string conversions](../concepts/strings.md#string-conversion-and-formatting) of other types.
...
- The maximum *plot count* for any script is 64. Each call to these functions contributes a different number to the total, depending on its arguments. See the [Plot limits](limitations.md#plot-limits) section of the [Limitations](limitations.md) page to learn more.
...
To learn more about the `plotshape()` and `plotchar()` functions and how their outputs differ from [labels](../visuals/text_and_shapes.md#labels), refer to the [Text and shapes](../visuals/text_and_shapes.md) page.
...
- All the placeholders with the `number` modifier in the `log.info()` call’s formatting string include the `0.00000` pattern, which forces the formatted numbers to always show five fractional digits. Refer to the [Formatting strings](../concepts/strings.md#formatting-strings) section of the [Strings](../concepts/strings.md) page for more information.
- The Pine Logs pane displays up to 10,000 historical logs. To view earlier logs, add another condition to the `if` structure that limits the `log.info()` call to specific bars. See the [Custom code filters](#custom-code-filters) section above for an example that restricts `log.*()` calls using [time inputs](../concepts/inputs.md#time-input).
...
All identifiers declared outside [user-defined functions](../language/user_defined_functions.md), [methods](../language/methods.md#user-defined-methods), [loops](../language/loops.md), [conditional structures](../language/conditional_structures.md), or [user-defined type](../language/type_system.md#user-defined-types) and [enum type](../language/type_system.md#enum-types) declarations belong to the *global scope*. Identifiers in the global scope are accessible to most *inner* (local) scopes after declaration. Every Pine script has exactly **one** global scope.
...
In Pine Script, any [user-defined function](../language/user_defined_functions.md) or [method](../language/methods.md#user-defined-methods) call, [loop](../language/loops.md), or [conditional structure](../language/conditional_structures.md) *returns* the result of the *final* expression or nested structure within its local scope. Scripts can use these structures’ returned results, excluding [void](../language/type_system.md#void), by assigning them to variables declared in the *outer* scope.

When debugging functions and conditional structures that contain multiple local variables, a common technique to extract data from their scopes is to return [tuples](../language/type_system.md#tuples) containing the data that requires inspection.

Here, we’ve modified the [previous](#extracting-data-from-local-scopes) example script’s `customMA()` function to return a tuple containing values calculated from the local scopes. With this change, the script can call the function with a *tuple declaration* to make all the data available to the global scope. The script plots the `q1Dbg` and `q3Dbg` values, highlights the background when `alphaDbg` is 0, and uses `log.info()` to display a formatted string containing all the extracted data in the [Pine Logs](#pine-logs) pane:
...
*Reference types*, including all [special types](../language/type_system.md#types) and [user-defined types (UDTs)](../language/type_system.md#user-defined-types), serve as structures for creating *objects*. Each object has an associated *reference* that distinguishes it and provides access to its data. Unlike fundamental types, variables of reference types **do not** store values directly. Instead, they hold the references for specific objects in memory.

An advanced, flexible way to extract data from local scopes is to initialize reference-type objects – such as instances of [collections](../language/type_system.md#collections) or UDTs – in the global scope and store local variable data in their elements or fields.

This technique is especially useful for extracting data from [user-defined functions](../language/user_defined_functions.md) and [methods](../language/methods.md#user-defined-methods). Although functions can access global variables, they cannot *reassign* them like global [conditional structures](../language/conditional_structures.md) and [loops](../language/loops.md) can. Consequently, they *cannot update* the data held by global variables of fundamental types. However, scripts do not modify reference types by reassigning their variables; they access objects via their references and use *methods* or *field reassignments* to update their data. As such, scripts *can* update global collections or UDT instances from inside function scopes.

For example, this modified version of our [initial script](#extracting-data-from-local-scopes) declares a global `debugData` variable that holds the reference of a `map` with “string” keys and “float” values. Each `map.put()` call inside the `customMA()` scope modifies the map by adding a *key-value pair* containing a local variable’s *name* and *value*. After calling `customMA()`, the script uses `map.get()` calls on `debugData` to retrieve the stored information for its debugging outputs:
...
[Loops](../language/loops.md) are structures that execute a local code block repeatedly based on a *counter* (`for`), the *contents* of a [collection](../language/type_system.md#collections) (`for…in`), or a *condition* (`while`). These structures allow scripts to perform repetitive tasks without redundant lines of code.

Because loops can execute their local code multiple times, programmers must use techniques to track local variables *across iterations* to debug them effectively. As with other structures, there are many ways to inspect loops. These sections cover two helpful techniques: [collecting loop information](#collecting-loop-information) and [tracing loop executions](#tracing-loop-executions).

#### Collecting loop information

One of the most effective loop inspection techniques is to use [collections](../language/type_system.md#collections) or [strings](../concepts/strings.md) to gather information from the local scope on each iteration requiring inspection, then use the information in output functions *after* the loop terminates.
...
- Scripts can generate [Pine Logs](#pine-logs) and [drawings](#pine-drawings) directly from within a loop’s local scope. However, because loops usually execute their local code *more than once*, calling `log.*()` or `label.new()` functions inside the scope can result in numerous logs or [labels](../visuals/text_and_shapes.md#labels) per bar. Logging on each iteration helps trace execution patterns, but it also limits the number of *historical* bars with available debug data. See the next section, [Tracing loop executions](#tracing-loop-executions), for an example.
- [Strings](../concepts/strings.md) can contain up to *40,960* characters, and large strings or repeated [concatenation](../concepts/strings.md#concatenation) can impact a script’s performance. Therefore, extracting loop information with string concatenation is suitable for relatively *small* loops or inspecting specific variables. To extract large amounts of data from loops, use [collections](../language/type_system.md#collections) instead.

#### Tracing loop executions

An alternative way to inspect a loop, without collecting information for use in the *outer scope*, is to add `log.*()` calls directly to the loop’s *local block*. Each iteration that activates the call results in a new message in the [Pine Logs](#pine-logs) pane, allowing programmers to trace the loop’s execution pattern in detail.

This simple script calculates a pseudorandom sample from a [binomial distribution](https://en.wikipedia.org/wiki/Binomial_distribution) using a `for` loop. The plotted `sample` series represents the number of `math.random()` calls across `trialsInput` iterations that return a value not exceeding the `probabilityInput` value. On each iteration where `success` is `false`, the loop *skips* the rest of its block and moves to the next iteration. On other iterations, it increments the `sample` value by one:
...
- The Pine Logs pane shows only the most recent 10,000 logs created on historical bars. Because this script creates multiple logs per bar, the earliest message in the pane is from less than 10,000 bars back. Programmers can use conditional logic that limits `log.*()` calls in order to inspect a loop’s execution flow on earlier bars with this technique. See the [Custom code filters](#custom-code-filters) section to learn more.

### Debugging collections

[Collections](../language/type_system.md#collections) are data structures that store values or references as *elements*, which scripts access using indices or keys, depending on the type. These structures can contain a lot of information, as the maximum number of elements across all instances of each collection type is 100,000.
...
- Creating a “string” representation of the collection with `str.tostring()` and displaying the result using [Pine Logs](#pine-logs) or other text outputs.
- Retrieving specific elements from the collection, then creating [formatted strings](../concepts/strings.md#formatting-strings) for logging, or using the element values or references in other output processes.

#### Displaying collection strings

The simplest way to inspect the data of [arrays](../language/arrays.md) and [matrices](../language/matrices.md) of “int”, “float”, “bool”, and “string” types is to generate “string” representations with the `str.tostring()` function, then display the results using [Pine Logs](#pine-logs) or other “string” outputs.
...
- The script calls `log.info()` on confirmed bars and `log.warning()` on the open bar. Users can [filter](#filtering-logs) the logs by [logging level](#logging-level) to inspect confirmed and unconfirmed bars’ logs separately.
- For larger collections whose “string” representations *exceed* 40,960 characters or cause excessive memory use, programmers can split them into *smaller parts* and convert them to strings separately. Alternatively, they can inspect [individual elements](#inspecting-individual-elements) via the `*.get()` method or `for…in` loops.

> **Tip:** In contrast to [arrays](../language/arrays.md) and [matrices](../language/matrices.md) of numeric values, Boolean values, or strings, [maps](../language/maps.md) of such types *do not* have built-in “string” representations. However, programmers can inspect a map’s contents with this technique by using `map.keys()` and `map.values()` to retrieve *key* and *value* arrays, then calling `str.tostring()` to convert those arrays to “string” values.

#### Inspecting individual elements

Collections of “color” or non-fundamental types (e.g., [labels](../visuals/text_and_shapes.md#labels)) *do not* have built-in “string” representations. Consequently, the technique described in the [Displaying collection strings](#displaying-collection-strings) section does not work for them.
...
When a script creates [labels](../visuals/text_and_shapes.md#labels), it *automatically* maintains an `array` containing each active label’s reference. Programmers can access this array using the `label.all` variable, and thus inspect each individual label’s properties on any bar.
...
[User-defined types (UDTs)](../language/type_system.md#user-defined-types) define the structures of [objects](../language/objects.md). Objects contain a fixed set of *fields*, where each field can hold a separate value or reference to another specified type, even to another instance of the *same* user-defined type.
...
To verify and understand the script’s calculations, we can *extract* information from the `Data` object’s *fields* and inspect the data with [Pine Logs](#pine-logs) or other outputs.

The script version below includes a `log.info()` call inside the `if` structure. The call displays [formatted text](../concepts/strings.md#formatting-strings) representing information from the `Data` object’s `prices`, `times`, and `length` fields in the Pine Logs pane. Now, we can view each change to the object’s data to confirm the script’s behavior:
...
- The script calls `log.info()` on confirmed bars and `log.warning()` on open bars, allowing users to [filter](#filtering-logs) the results by [logging level](#logging-level) in the Pine Logs pane.
...
Below are a few helpful coding recommendations based on our [Style guide](style_guide.md) and best practices:

- Follow the [script organization](style_guide.md#script-organization) guidelines. Organizing scripts based on this structure makes different parts of the code simple to locate and inspect.
- Use *identifiers* that you can read, distinguish, and understand. When a code contains unclear identifiers, it is often harder to debug efficiently. See our [Naming conventions](style_guide.md#naming-conventions) to learn our recommended identifier format.
