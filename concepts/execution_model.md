...
-   This script can execute  _more than four_  times per bar if it uses  high [Bar detalization](https://www.tradingview.com/pine-script-docs/concepts/strategies/#adjusting-historical-bar-detail)  (formerly Bar Magnifier; `use_bar_magnifier = true`), because this setting enables the broker emulator to fill orders on historical bars using intrabar prices from a  _lower timeframe_.
...
-   An alternative way to confirm the script’s increased execution count is to select and clear the “On order fill” checkbox in the strategy’s Script execution settings while  [profiling](https://www.tradingview.com/pine-script-docs/writing/profiling-and-optimization/#profiling-a-script)  the code.
...
If the  [strategy()](https://www.tradingview.com/pine-script-reference/v6/#fun_strategy)  statement includes  `calc_on_every_tick = true`, or if the user selects “On realtime bar tick” in the strategy’s Script execution settings, the script executes on a realtime bar after  _each new update_  from the data feed, similar to an indicator.  Likewise, `calc_on_every_history_tick = true` (or “On history bar tick”, added July 2026) makes the script execute on each available tick of every _historical_ bar, with `high`, `low`, `close`, and `volume` updating per tick.

Additionally, if the  [strategy()](https://www.tradingview.com/pine-script-reference/v6/#fun_strategy)  statement includes  `calc_on_order_fills = true`  or the user selects “On order fill” in the strategy’s Script execution settings, the script executes on  _each tick_  where the  [broker emulator](https://www.tradingview.com/pine-script-docs/concepts/strategies/#broker-emulator)  fills an order. With this behavior, the system can execute the script multiple times on the open bar, but only on the ticks where an  _order fill_  occurs.
