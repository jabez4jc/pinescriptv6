...
The following script plots a 20-bar [simple moving average (SMA)](https://www.tradingview.com/support/solutions/43000502589) using a call to the `ta.sma()` function. While it is straightforward to write, the code is not very *flexible* because the function call uses specific `source` and `length` arguments that users cannot change without modifying the code:
...
Scripts create input *widgets* in the “Inputs” tab that accept different types of inputs based on their `input.*()` function calls. By default, each input appears on a new line of the “Inputs” tab in the order of the `input.*()` calls. Programmers can also organize inputs in different ways by using the `input.*()` functions' `group` and `inline` parameters. See [this section](#input-function-parameters) below for more information.

Our [Style guide](../writing_scripts/style_guide.md#style-guide) recommends placing `input.*()` calls at the beginning of the script.
...
Since an `input.*()` call is simply another function call in Pine Script, programmers can combine them with [arithmetic](../language/operators.md#arithmetic-operators), [comparison](../language/operators.md#comparison-operators), [logical](../language/operators.md#logical-operators), and [ternary](../language/operators.md#-ternary-operator) operators to assign expressions to variables. This simple script compares the result from a call to `input.string()` to the “On” string and assigns the result to the `plotDisplayInput` variable. This variable is of the “input bool” type because the `==` operator returns a “bool” value:
...
All values returned by `input.*()` functions except “source” ones are “input” qualified values. See our User Manual's section on [type qualifiers](../language/type_system.md#qualifiers) for more information.
...
Most input parameters require “const” arguments. However, two parameters allow values with stronger [qualifiers](../language/type_system.md#qualifiers): the `active` parameter of all `input*()` functions accepts an “input bool” value, and the `defval` parameter of `input.source()` accepts a “series float” value.

The parameters that require “const” arguments *cannot* use dynamic values or results from other `input*()` calls as arguments, because “input” and other qualifiers are *stronger* than the “const” qualifier. See the [Type system](../language/type_system.md) page for more information.

Let’s examine each parameter:

`defval`

The default value assigned to the input variable, and the initial value that appears in the input widget. It is the first parameter of all `input*()` functions. The required type for a `defval` argument depends on the input function type, e.g., an “int” `defval` argument for `input.int()`, a “string” `defval` argument for `input.string()`, etc. The [generic input](#generic-input) function infers its input type based on the `defval` argument used in the `input()` call.
...
A list specifying the possible values that this input can have. This parameter accepts a [tuple](../language/type_system.md#tuples), which is a comma-separated list of elements enclosed in square brackets (e.g., `["ON", "OFF"]`, `[1, 2, 3]`, `[myEnum.On, myEnum.Off]`). These elements appear in a dropdown widget, from which users can select only one value at a time. If an input uses the `options` parameter, the `defval` value must be one of the list’s elements.

`minval`

The minimum valid value for the input field in an [integer input](#integer-input) or [float input](#float-input).

`maxval`

The maximum valid value for the input field in an [integer input](#integer-input) or [float input](#float-input).

`step`

The increment by which the field’s value changes when clicking the up/down arrows in an [integer input](#integer-input) or [float input](#float-input) widget. The default `step` value is 1.
...
Using `confirm = true` for a [time input](#time-input) or [price input](#price-input) enables an interactive input mode where users can click on the chart to set time and price values.
...
The simple script below contains two `input.string()` calls. The first call creates a text field for defining the `timezone` argument of two `str.format_time()` calls. It allows users to supply any text representing a [time zone](time.md#time-zones) in *UTC-offset* or *IANA* formats. The second call creates a *dropdown* input with three preset options that determine the text shown in the drawn [labels](../visuals/text_and_shapes.md#labels) (`"Open time"`, `"Close time"`, or `"Both"`):
...
- An alternative way to provide a strict list of input options is to use an [enum input](#enum-input), which constructs a dropdown menu based on the *members* of an [enum type](../language/type_system.md#enum-types).
- In contrast to string declarations in code, the text field from a string input treats an input backslash (`\`) as a *literal character*. Therefore, the `input.string()` function *does not* parse input [escape sequences](strings.md#escape-sequences) such as `\n`.
...
- The script can use `request.security()` within a loop because [dynamic requests](other_timeframes_and_data.md#dynamic-requests) are enabled by default.
- As with `input.string()`, the `input.text_area()` function’s text field treats backslashes (`\`) as literal characters. It cannot process [escape sequences](strings.md#escape-sequences). However, the field automatically parses any line terminators and tab spaces in the specified text.
- Because text area inputs allow freeform, multiline text, it is often helpful to validate the `input.text_area()` function’s results to prevent erroneous user inputs. Refer to the [Matching patterns](strings.md#matching-patterns) section of the [Strings](strings.md) page for an example that confirms an input symbol list using [regular expressions](https://en.wikipedia.org/wiki/Regular_expression).

### Timeframe input

The `input.timeframe()` function creates a dropdown input containing *timeframe choices*. It returns a “string” value representing the selected timeframe in our [specification format](timeframes.md#timeframe-string-specifications), which scripts can use in `request.*()` calls to retrieve data from user-selected timeframes.
...
- By default, the `input.timeframe()` call’s dropdown contains options for the chart’s timeframe and all timeframes listed in the chart’s “Time interval” menu. To restrict the available options to specific preset timeframes, pass a [tuple](../language/type_system.md#tuples) of timeframe strings to the function’s `options` parameter.
- This script calls `runtime.error()` to raise a custom runtime error if the `timeframe.in_seconds()` value for the `tfInput` timeframe is *less* than the number of seconds in the main timeframe, preventing it from requesting lower-timeframe data. See [this section](other_timeframes_and_data.md#higher-timeframes) of the [Other timeframes and data](other_timeframes_and_data.md) page to learn more.
- The `request.security()` call uses `barmerge.lookahead_on` as its `lookahead` argument, and it offsets the `expression` argument by one bar when the `tfInput` represents a *higher timeframe* to [avoid repainting](other_timeframes_and_data.md#avoiding-repainting).
...
The value returned by `input.session()` is a valid string in session format. See the manual's page on [sessions](sessions.md) for more information.
...
- Users can *pair* time inputs with [price inputs](#price-input) to create interactive chart points. See the next section to learn more.
...
- The script creates the drawing by constructing an `array` of [chart points](../language/type_system.md#chart-points), then using that array in a `polyline.new()` call. Refer to the [Polylines](../visuals/lines_and_boxes.md#polylines) section of the [Lines and boxes](../visuals/lines_and_boxes.md) page to learn more about polyline drawings.

### Enum input

The `input.enum()` function creates a dropdown input that displays *field titles* corresponding to distinct *members* (possible values) of an [enum type](../language/type_system.md#enum-types). The function returns one of the unique, named values from a declared [enum](../language/enums.md), which scripts can use in calculations and logic requiring more strict control over allowed values and operations. Supply a list of enum members to the `options` parameter to specify the members users can select from the dropdown. If one does not specify an enum field's title, its title is the “string” representation of its *name*.

This example declares a `SignalType` enum with four fields representing named signal display modes: `long`, `short`, `both`, and `none`. The script uses a member of this [enum type](../language/type_system.md#enum-types) as the `defval` argument in the `input.enum()` call to generate a dropdown in the “Inputs” tab, allowing users to select one of the enum's titles to control which signals it displays on the chart:
