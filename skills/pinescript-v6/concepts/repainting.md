...
- **Unacceptable**: Scripts that leak future information into the past, strategies that execute on [non-standard charts](non_standard_charts_data.md), and scripts using realtime intrabars to generate alerts or orders, are examples that can produce heavily misleading repainting behavior.
...
Whatever your design decisions are, if you [publish](../writing_scripts/publishing.md) your script, explain the script to traders so they can understand how it behaves.
...
Let's look at this simple script. It detects crosses of the `close` value (in the realtime bar, this corresponds to the current price of the instrument) over and under an [EMA](https://www.tradingview.com/support/solutions/43000592270):
...
- Unlike a plain `request.security()` call, a call to this wrapper function cannot use a tuple as the `expression` argument. For multi-element requests, programmers can pass the ID of an [object](../language/objects.md) of a [user-defined type](../language/type_system.md#user-defined-types) whose *fields* contain the desired elements.
...
For more reliable lower-timeframe data requests, use `request.security_lower_tf()`, as explained in [this](other_timeframes_and_data.md#lower-timeframe-data) section of the [Other timeframes and data](other_timeframes_and_data.md) page.
...
Scripts begin executing on the chart's first historical bar, and then execute on each bar sequentially, as is explained in this manual's page on Pine Script's [execution model](../language/execution_model.md). If the first bar changes, then the script will often not calculate the same way it did when the dataset began at a different point in time.
