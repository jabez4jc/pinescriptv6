...
Both these errors occur when creating objects too distant from the current bar. An x point on a [line](../visuals/lines_and_boxes.md#lines), [label](../visuals/text_and_shapes.md#labels), or [box](../visuals/lines_and_boxes.md#boxes) can not be more than 9999 bars in the past or more than 500 bars in the future relative to the bar on which the script draws it.
...
Scripts can give deceptive output if they [repaint](../concepts/repainting.md) by behaving differently on historical and elapsed realtime bars. This type of repainting is most commonly caused by requesting data from another context using the `request.security()` function.
...
- Set alerts to fire on bar close. Read more about repainting alerts in the FAQ entry [Why is my alert firing at the wrong time?](alerts.md#why-is-my-alert-firing-at-the-wrong-time)
...
Various boolean [built-in](../language/built_ins.md) variables within the `chart.*` namespace enable a script to detect the type of chart it is running on.
...
The following example script uses functions from this library to create two horizontal [lines](../visuals/lines_and_boxes.md#lines) on the chart, signifying the highest and lowest price points within the range of visible bars. The script draws [labels](../visuals/text_and_shapes.md#labels) for these lines, displaying both the price and the corresponding timestamp for each high and low point. As the chart is manipulated through scrolling or zooming, these lines and labels dynamically update to reflect the highest and lowest values of the newly visible bars:
...
- Because the visible chart values are defined in the [global scope](programming.md#what-does-scope-mean), *outside* the local block defined by `barstate.islast`, the functions process the entire dataset before determining the final high and low values.
...
If scripts include the [`timeframe`](../language/declaration_statements.md#timeframe-and-timeframe_gaps) parameter in the `indicator()` declaration, the user can choose the timeframe in which the script runs. The script can set a default timeframe.
...
- Scripts that use the `timeframe` parameter can plot values quite differently depending on which settings are chosen. For an explanation, see the [Leveraging multi-timeframe analysis](https://www.tradingview.com/support/solutions/43000591555/) article in our Help Center.
...
The simplest and most efficient method is to use the built-in `math.sum()` function, and pass it a conditional series to count. This function maintains a running total of the count as each bar is processed, and can take a [simple](../language/type_system.md#simple) or [series](../language/type_system.md#series) length.

The following example script uses both of these calculation methods. It also uses a “series” length that adjusts for the first part of the chart, where the number of bars available is less than the length. This way, the functions do not return [`na` values](../language/type_system.md#na-value).
...
- The script uses an [enum](../language/enums.md#enums) variable with three possible values to store the type of the last pivot and to decide whether to confirm subsequent pivots.
- A single boolean value cannot reliably do this, because boolean values [can only be `true` or `false` and not `na`](../migration_guides/to_pine_version_6.md#boolean-values-cannot-be-na). Using a boolean value can cause unexpected behavior, for example, at the beginning of the chart history where no trigger condition has occurred.
- A pair of boolean variables can replicate this behavior, with careful handling. See the FAQ entry [“How can I accumulate a value for two exclusive states?”](#how-can-i-accumulate-a-value-for-two-exclusive-states) for an example of using two boolean values in this way.
- A string variable can also do the same thing. The advantage of an [enum](../language/enums.md#enums) over a string is that all possible allowed values are known, thus avoiding the case where a condition tests for a value that is misspelled, outdated or otherwise not relevant. Such a test silently fails in every possible case, and the corresponding logic never runs. Such tests can therefore cause bugs that are difficult to find.
...
> **Notice:** If the individual indicators are large or computationally complex, programmers might encounter issues with one or more of Pine’s [limitations](../writing_scripts/limitations.md) when combining them into a single script.
...
The example script below uses a `rescale()` function to rescale RSI values, and a `normalize()` function to normalize [Commodity Channel Index (CCI)](https://www.tradingview.com/support/solutions/43000502001-commodity-channel-index-cci/) and `volume` values. Although normalizing is an imperfect solution, it is more complete than using `ta.lowest()` and `ta.highest()`, because it uses the minimum and maximum values for the complete set of elapsed bars instead of a subset of fixed length.
...
Programmers can measure the time that a script takes to run and see detailed information about which parts of the code take longest in the Pine Profiler. See the section of the User Manual on [Profiling and optimization](../writing_scripts/profiling_and_optimization.md) for more information.

## How can I save a value when an event occurs?

To save a value when an event occurs, use a *persistent variable*. Scripts declare persistent variables by using the [var](../language/variable_declarations.md#var) keyword. Such variables are initialized only once, at `bar_index` zero, instead of on each bar, and maintain the same value after that unless changed.

In the following example script, the [var](../language/variable_declarations.md#var) keyword allows the `priceAtCross` variable to maintain its value between bars until a crossover event occurs, when the script updates the variable with the current close price. The [:=](../language/operators.md#-reassignment-operator) reassignment operator ensures that the global variable `priceAtCross` is modified. Using the [=](../language/operators.md#-assignment-operator) assignment operator instead would create a new local variable that is inaccessible outside the [if](../language/conditional_structures.md#if-structure) block. The new local variable would have the same name as the global variable, which is called *shadowing*. The compiler warns about shadow variables.
...
Optimizing Pine Script code can make scripts run faster and use less memory. For large or complex scripts, optimization can avoid scripts reaching the [computational limits](../writing_scripts/limitations.md).

The [Pine Profiler](../writing_scripts/profiling_and_optimization.md) analyzes all significant code in a script and displays how long each line or block takes to run. Before optimizing code, run the Pine Profiler to identify which parts of the code to optimize first. The Pine Profiler section of the User Guide contains an extensive discussion of [how to optimize code](../writing_scripts/profiling_and_optimization.md#optimization). In addition, consider the following tips:

- Use strategy scripts only to model trades. Otherwise, use indicator scripts, which are faster.
- Become familiar with the Pine [execution model](../language/execution_model.md) and [time series](../language/execution_model.md#time-series) to structure code effectively.
- Declare variables with the [var](../language/variable_declarations.md#var) keyword when initialization involves time-consuming operations like complex functions, arrays, objects, or string manipulations.
...
- Using built-in functions is usually faster than writing custom functions that do the same thing. Sometimes, alternative logic can be more efficient than using standard functions. For example, use a persistent variable when an event occurs, to avoid using `ta.valuewhen()`, as described in the FAQ entry [How can I save a value when an event occurs?](#how-can-i-save-a-value-when-an-event-occurs). Or save the `bar_index` when a condition occurs to avoid using `ta.barssince()`, as described in the FAQ entry [How to remember the last time a condition occurred?](#how-to-remember-the-last-time-a-condition-occurred).
...
To sum a series of values, initialize a persistent variable by using the [var](../language/variable_declarations.md#var) keyword to the track the sum. Then use a logical test to reset the values when a condition occurs.
...
- In the `ta.macd()` function call, we only require two of the three values returned in the [tuple](../language/type_system.md#tuples). To avoid unnecessary variable declarations, we assign the third tuple value to an underscore. Here, [the underscore acts like a dummy variable](../language/variable_declarations.md#using-an-underscore-_-as-an-identifier).
...
There are different ways to code this kind of logic. See the FAQ entry [“How can I alternate conditions”](#how-can-i-alternate-conditions) for an example of using an [enum](../language/enums.md#enums) to manage two exclusive states. The following example script uses two boolean variables to do the same thing.
...
A script can use `plot*()` functions and other [plot visuals](../visuals/overview.md#plot-visuals) only in the [global scope](programming.md#what-does-scope-mean) – they cannot be included in the local scopes of [conditional structures](../language/conditional_structures.md), [loops](../language/loops.md), or [user-defined functions](../language/user_defined_functions.md) and [methods](../language/methods.md#user-defined-methods). Therefore, plots can only use variables and literals that are declared globally.

However, programmers can [extract data from local scopes](../writing_scripts/debugging.md#extracting-data-from-local-scopes) to the global scope to make the data accessible to `plot*()` functions. Assign the local scope values to globally declared variables, using [return expressions](../writing_scripts/debugging.md#extraction-using-return-expressions) or [reference types](../writing_scripts/debugging.md#extraction-using-reference-types), then use these global variables in `plot*()` calls to visualize the local data.

Alternatively, use [Pine Logs](../writing_scripts/debugging.md#pine-logs) or [drawings](../writing_scripts/debugging.md#pine-drawings) to display values from within local scopes directly.
