...
This compiler warning occurs if a call to a [built-in function](../language/built_ins.md#built-in-functions) or [user-defined function](../language/user_defined_functions.md) (or [method](../language/methods.md)) inside a [conditional structure](../language/conditional_structures.md) or [loop](../language/loops.md) retrieves data from its calculations on *past bars* by using the [`[]` history-referencing operator](../language/operators.md#-history-referencing-operator) or other functions that rely on history internally. History-dependent function calls that execute either conditionally or iteratively can cause **unintended results**. A similar warning also occurs if a [ternary](../language/operators.md#-ternary-operator) or `and`/`or` operation executes a history-dependent function call conditionally.

As a script runs a dataset, Pine’s runtime system successively *commits (saves)* the data for the script’s variables and expressions on each closed bar to internal [time series](../language/execution_model.md#time-series) structures. For the system to save *consistent* time series data for a function call, the call must execute **once** on **each** bar’s close. If the call does not execute on each bar, or if it executes more than once on some bars, the time series for the call contains an **inconsistent** history of past data.
...
> **Note:** Referencing the history of a local variable declared inside conditional structures or loops causes a slightly different compiler warning, but the solution is similar. Move the [variable declaration](../language/variable_declarations.md) to the *global scope* to ensure consistent history-referencing operations on that variable.
...
For example, this script version executes the `previousValue()` call in the global scope and [reassigns](../language/variable_declarations.md#variable-reassignment) the result to the `pastValue` variable inside the `if` structure. Now, on bars where the plot appears, the result consistently shows the `bar_index` value from one bar back:
...
> **Tip:** For more *advanced* details about this behavior, refer to the [Time series in scopes](../language/execution_model.md#time-series-in-scopes) section of the [Execution model](../language/execution_model.md) page.

### Why this behavior?

This behavior is necessary because forcing a function call to execute once on each bar, regardless of scope, would cause erroneous or unexpected results, especially in functions that produce *side effects*, such as [drawings](../visuals/overview.md#drawing-visuals), [alerts](../concepts/alerts.md), and [Pine Logs](../writing_scripts/debugging.md#pine-logs), or functions that use persistent variable [declaration modes](../language/variable_declarations.md#declaration-modes).
...
If the use of a function call in a local block does not cause a compiler warning, it is typically safe to use the call in that block without affecting the call’s calculations. However, if the warning occurs, move the function call to the global scope and outside [ternary](../language/operators.md#-ternary-operator) or `and`/`or` operations to ensure consistency. If you choose to keep a function call within a local block or a conditional expression despite encountering a warning, [debug](../writing_scripts/debugging.md) the script carefully to avoid unintended results.
