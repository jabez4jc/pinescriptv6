...
Before an object can be created, its type must be defined. The [User-defined types](type_system.md#user-defined-types) section of the [Type system](type_system.md) page explains how to do so.
...
When using the `var` keyword while declaring a variable assigned to an object of a [user-defined type](type_system.md#user-defined-types), the keyword automatically applies to all the object's fields:
...
The value of an object's fields can be changed using the [:=](operators.md#-reassignment-operator) reassignment operator.
...
Pine Script *collections* ([arrays](arrays.md), [matrices](matrices.md), and [maps](maps.md)) can contain references to UDT objects, enabling programmers to add virtual dimensions to their data structures. To create a collection of a user-defined type, call the collection type’s `*.new*()` function with the UDT name in the function’s *type template*.
...
To explicitly declare the type of a variable as an `array`, `matrix`, or `map` of a [user-defined type](type_system.md#user-defined-types), prefix the variable declaration with collection’s *type keyword* followed by its *type template*. For example:
...
See the [Collections](type_system.md#collections) section of the [Type system](type_system.md) page to learn about type templates.
...
In the following example, we have defined an `InfoLabel` type with a label as one of its fields. The script instantiates a `shallow` copy of the `parent` object, then calls a user-defined `set()` [method](methods.md) to update the `info` and `lbl` fields of each object. Since the `lbl` field of both objects points to the same label instance, changes to this field in either object affect the other:
...
However, scripts cannot use the following keywords for [fundamental types](type_system.md#types) as names for UDTs: `int`, `float`, `string`, `bool`, and `color`.
