# Operators (arithmetic, comparison, logical, ternary ?:, history [], assignment :=)
...
**Returns:** Binary `+` for strings returns concatenation of expr1 and expr2 For numbers returns integer or float value, or series of values: Binary `+` returns expr1 plus expr2. Unary `+` returns expr (does nothing added just for the symmetry with the unary - operator).
...
**Returns:** Returns integer or float value, or series of values: Binary `-` returns expr1 minus expr2. Unary `-` returns the negation of expr.
...
You can learn more about user-defined functions in the User Manual's pages on [Declaring functions](../language/user_defined_functions.md) and [Libraries](../concepts/libraries.md).
